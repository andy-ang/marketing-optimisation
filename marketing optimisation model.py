...
# import data
# =============================================================================
soc_dem = pd.read_excel('DataScientist_CaseStudy_Dataset.xlsx', sheet_name='Soc_Dem', index_col = 'Client')
pdt_acct_bal = pd.read_excel('DataScientist_CaseStudy_Dataset.xlsx', sheet_name='Products_ActBalance', index_col = 'Client')
inflow_outflow = pd.read_excel('DataScientist_CaseStudy_Dataset.xlsx', sheet_name='Inflow_Outflow', index_col = 'Client')
sales_rev = pd.read_excel('DataScientist_CaseStudy_Dataset.xlsx', sheet_name='Sales_Revenues', index_col = 'Client')
...
for target in target_cols:
    ...
    joblib.dump(random_search.best_params_, target+'_params.pkl')
...
for target in target_cols:
    # load best params
    xgb_params = joblib.load(target+'_params.pkl')
    ...
    # save model
    joblib.dump(xgb_model, target+'_uncali_model.pkl')
    joblib.dump(cali_xgb_model, target+'_cali_model.pkl')
...
for target in target_cols:
    ...
    print(target, np.mean(sales_rev[sales_rev[rev_col]>0][rev_col]))
    xgb_model = joblib.load(target+'_cali_model.pkl')
...
pred_prob.to_csv('test_recommendation.csv')
